...
class AbsensiController extends Controller
{
    public function index(Request $request)
    {
        ...
        $mahasiswas = $instansi->mahasiswas()->orderBy('nama')->get();

        $query = Absensi::with(['mahasiswa'])
            ->whereHas('mahasiswa', fn($q) => $q->where('instansi_id', $instansi->id))
            ->orderByDesc('tanggal')
        ...
        return view('instansi.absensi.index', compact('instansi', 'mahasiswas', 'absensis'));
    }
    ...
    public function show(Mahasiswa $mahasiswa, Request $request)
    {
        ...
        // Pastikan mahasiswa KP di instansi yang sedang login
        abort_if($mahasiswa->instansi_id !== $instansi->id, 403, 'Mahasiswa ini bukan KP di instansi Anda.');

        $mahasiswa->load(['dosen']);
        ...
        return view('instansi.absensi.show', compact('mahasiswa', 'absensis', 'stats', 'instansi'));
    }
}