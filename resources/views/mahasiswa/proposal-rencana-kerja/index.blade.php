...
<div class="card"><div class="card-body">
  ...
  @if($proposal)
    <div class="alert {{ $proposal->status === 'revisi' ? 'alert-danger' : 'alert-info' }}">
      Status <span class="badge {{ ['menunggu'=>'badge-proses','disetujui'=>'badge-selesai','revisi'=>'badge-rejected'][$proposal->status] }}">{{ ucfirst($proposal->status) }}</span>
      — berkas <a href="{{ $proposal->file_url }}" target="_blank">{{ $proposal->file_asli }}</a>@if($proposal->uploaded_at) dikirim {{ $proposal->uploaded_at->format('d M Y, H:i') }}@endif.
      @if($proposal->catatan)<br><strong>Catatan dosen:</strong> {{ $proposal->catatan }}@endif
    </div>
  @endif
