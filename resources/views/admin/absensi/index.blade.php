...
<div class="page-header page-header-row">
  <div>
    ...
    <p>Pantau kehadiran seluruh mahasiswa KP di semua instansi.</p>
  </div>
</div>
  ...
  <div class="card-body" style="padding:14px 18px">
    <form method="GET" style="display:flex;flex-wrap:wrap;gap:10px;align-items:flex-end">
      ...
      <div style="flex:1;min-width:140px">
        ...
        <select name="dosen_id" class="form-control form-select">
      ...
      <div style="flex:1;min-width:140px">
        ...
        <select name="instansi_id" class="form-control form-select">
          <option value="">Semua Instansi</option>
          @foreach($instansis as $i)
            <option value="{{ $i->id }}" {{ request('instansi_id') == $i->id ? 'selected' : '' }}>
              {{ $i->nama }}
            </option>
          @endforeach
        </select>
      ...
      <div style="display:flex;gap:8px">
        ...
        <a href="{{ route('admin.absensi.index') }}" class="btn btn-outline btn-sm">Reset</a>
      </div>
    </form>
  </div>
</div>
    ...
    <table>
      ...
      <tbody>
        ...
        <tr style="{{ $perluTinjau ? 'background:rgba(239,68,68,.04)' : '' }}">
          ...
          <td class="text-sm">{{ $a->mahasiswa->instansi?->nama ?? '–' }}</td>
          <td class="text-sm">
          ...
          <td style="text-align:center">
            @if($a->jam_masuk)
              ...
              <div class="text-sm text-muted">{{ $a->jarak_masuk }}m</div>
          ...
          <td style="text-align:center">
            @if($a->jam_keluar)
              ...
              <div class="text-sm text-muted">{{ $a->jarak_keluar }}m</div>
          ...
          <td style="text-align:center">
            ...
            @else
              <span class="badge badge-belum">Tidak Hadir</span>
            @endif
          </td>
          <td style="text-align:center">
            @if($a->catatan_dosen)
          ...
          <td style="text-align:center">
            <a href="{{ route('admin.absensi.show', $a->mahasiswa) }}" class="btn btn-ghost btn-sm">Lihat</a>
          </td>
        </tr>
        @empty
        <tr>
