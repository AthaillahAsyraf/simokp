@extends('layouts.app')
@section('title','Absensi Mahasiswa KP')
@section('content')

<div class="page-header page-header-row">
  <div>
    <h1>📋 Absensi Mahasiswa KP</h1>
    <p>Rekap kehadiran mahasiswa KP di {{ $instansi->nama }}.</p>
  </div>
</div>
  ...
  <div class="card-body" style="padding:14px 18px">
    <form method="GET" style="display:flex;flex-wrap:wrap;gap:10px;align-items:flex-end">
      ...
      <div style="display:flex;gap:8px">
        ...
        <a href="{{ route('instansi.absensi.index') }}" class="btn btn-outline btn-sm">Reset</a>
      </div>
    </form>
  </div>
</div>
...
<div class="card">
        ...
        <tr style="{{ $perluTinjau ? 'background:rgba(239,68,68,.04)' : '' }}">
          ...
          <td style="text-align:center">
            @if($a->jam_masuk)
              ...
              @else
                <span style="font-size:11px;color:var(--green-600)">✓ Valid</span>
              @endif
  ...
  <div style="padding:10px 18px;border-top:1px solid var(--gray-100);display:flex;gap:16px;flex-wrap:wrap;font-size:12px;color:var(--gray-500)">
    ...
    <span>⚠️ Ada absen di luar radius</span>
