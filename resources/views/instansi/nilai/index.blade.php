    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            @if($m->pembimbing_lapangan_nama)
              <button class="btn btn-outline btn-xs" onclick="openNilai({{ $m->id }}, {{ json_encode($m->nama) }}, {{ json_encode($m->pembimbing_lapangan_nama) }}, {{ json_encode($n?->nilai_lapangan) }}, {{ json_encode($n?->catatan_lapangan) }})">Input Nilai</button>
            @else
        ...
        @empty
          <tr><td colspan="9" style="text-align:center;padding:28px;color:var(--gray-400)">Belum ada mahasiswa KP di instansi Anda.</td></tr>
        @endforelse
      </tbody>
    </table>
  </div>
  ...
  <div class="modal-box">
    ...
    <form method="POST" id="nilaiForm">
      ...
      <div class="form-group">
        ...
        <input type="number" name="nilai_lapangan" id="nNilai" class="form-control" min="0" max="100" step="0.01" required>
      </div>
      <div class="form-group">
        <label class="form-label">Catatan</label>
        <textarea name="catatan_lapangan" id="nCatatan" class="form-control" rows="3" placeholder="Masukan/feedback untuk mahasiswa..."></textarea>
      </div>
...
@push('scripts')
<script>
function openNilai(id, nama, namaPic, nilai, catatan) {
