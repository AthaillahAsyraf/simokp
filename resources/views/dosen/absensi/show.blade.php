...
<div class="page-header page-header-row">
  <div style="display:flex;align-items:center;gap:12px">
    <a href="{{ route('dosen.absensi.index') }}" class="btn btn-outline btn-sm">← Kembali</a>
    <div>
      <h1>📋 Absensi – {{ $mahasiswa->nama }}</h1>
      <p><code>{{ $mahasiswa->nim }}</code> · KP di {{ $mahasiswa->instansi?->nama ?? '–' }}</p>
    </div>
  </div>
</div>
...
<div class="stats-grid stats-3" style="margin-bottom:18px">
  <div class="stat-card c-dosen">
    ...
    <div class="stat-sub">Hari dengan absen masuk</div>
  ...
  <div class="stat-card" style="border-left:3px solid #dc2626">
    ...
    <div class="stat-sub">Luar radius GPS</div>
...
<div class="card" style="margin-bottom:14px">
  <div class="card-body" style="padding:12px 18px">
    <form method="GET" style="display:flex;gap:10px;align-items:flex-end;flex-wrap:wrap">
      ...
      <a href="{{ route('dosen.absensi.show', $mahasiswa) }}" class="btn btn-outline btn-sm">Semua</a>
    </form>
  </div>
</div>
      ...
      <tbody>
        ...
        <tr style="{{ $perluTinjau ? 'background:rgba(239,68,68,.05)' : '' }}">
          ...
          <td style="min-width:200px">
            <form action="{{ route('dosen.absensi.catatan', $a) }}" method="POST" style="display:flex;gap:6px;align-items:center">
              @csrf @method('PATCH')
              ...
              <button type="submit" class="btn btn-ghost btn-sm" title="Simpan">💾</button>
            </form>
          </td>
        </tr>
        @empty
